...
const ROUTES = {
  ...
  '/register':   { view:'register',   protected:false, navKey:null,        title:'Customer Registration', chrome:false },
...
const NAV_ITEMS = [
  ...
  { path:'more',       navKey:'more',      icon:'⋯', label:'More' }
];
...
function renderBottomNav(activeKey){
  ...
  nav.innerHTML = NAV_ITEMS.map(item => `<button class="bottomnav__item ${item.navKey===activeKey?'active':''}" data-nav="${item.navKey}" data-path="${item.path}">
...
function openMoreSheet(){
  ...
  CouponUtils.openModal(`
    ...
    <div class="stack">
      ${items.map(i => `<button class="list-item" style="width:100%;background:none;border:none;text-align:left;cursor:pointer;" data-path="${i.path}">
